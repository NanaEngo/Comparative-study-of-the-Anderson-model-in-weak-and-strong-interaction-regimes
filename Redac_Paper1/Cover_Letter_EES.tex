...
\textbf{(3) Actionable design principles}: Using Pareto frontier analysis, we establish quantitative specifications for next-generation OPV materials—specifically, dual-band transmission at \SIlist{750;820}{\nano\meter} with \SI{70}{\nano\meter} FWHM. These specifications balance electrical power conversion efficiency (\SIrange{15}{21}{\percent}) with biological energy transfer enhancement (\SIrange{8}{25}{\percent}).

\textbf{(4) Global applicability}: Geographic simulations across nine climate zones—including five sub-Saharan African sites spanning equatorial humid, tropical savanna, and Sahel climates—confirm persistent quantum advantages of \SIrange{18}{26}{\percent}. This broad validation, which includes regions where food--energy co-production is most urgently needed, demonstrates worldwide deployment potential.

\textbf{(5) Experimental verification pathway}: We provide specific predictions for verification using existing experimental techniques, including coherence lifetime extensions of \SIrange{20}{50}{\percent} in ultrafast spectroscopy and \SIrange{10}{18}{\percent} yield improvements in field trials.
...
\begin{itemize}
    ...
    \item \textbf{Sustainable energy}: Agrivoltaics are key to sustainable land use, contributing to UN SDGs (Clean Energy, Zero Hunger, Climate Action), with particular relevance to food security in sub-Saharan Africa.
