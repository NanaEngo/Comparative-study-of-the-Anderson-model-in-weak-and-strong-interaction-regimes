...
Density functional theory (DFT) calculations employ the B3LYP hybrid exchange-correlation functional with a 6-31G(d,p) double-zeta polarised basis set, using Gaussian~16 or ORCA~5.0. Convergence criteria are set to $10^{-8}$~\si{\hartree} for the SCF procedure and $10^{-5}$~\si{\hartree\per\bohr} for geometry optimisation.
...
The coupling matrix $J_{nm}$ (\si{\per\cm}, symmetric) from the refined Cho \textit{et al.}\ parameterisation is:
\begin{equation}
\bm{J} = \begin{pmatrix}
0 & -104 & 8 & -5 & 6 & -13 & -2 \\
-104 & 0 & 30 & 8 & 2 & 7 & 11 \\
...
Simulations employ Process Tensor decomposition with Low-Temperature Correction (PT-HOPS+LTC) to enhance computational efficiency at physiological temperatures. This section details the method and benchmarks its performance against HEOM and Redfield theory.

\begin{table}[ht]
\centering
\caption{\textbf{Computational performance comparison.} All benchmarks are for \SI{1}{\pico\second} of FMO dynamics at \SI{295}{\kelvin} with a Drude + vibronic bath on an AMD EPYC 7542 processor (32 cores at \SI{2.9}{\giga\hertz}). Redfield (Markovian) results are shown for comparison but fail to capture coherence effects. PT-HOPS+LTC achieves near-HEOM accuracy with a \num{10}$\times$ speedup, enabling large-scale simulations ($N > 20$ sites) intractable for HEOM.}
...
As shown in \cref{tab:computational_benchmarks}, PT-HOPS+LTC achieves a \num{10}-fold speedup over HEOM with ${<}\,\SI{2}{\percent}$ accuracy and exhibits near-linear scaling with system size for localised excitons, enabling simulations of complete photosynthetic antenna complexes (100+ chromophores) with non-Markovian accuracy.

\subsection{Pad\'e decomposition of the bath correlation function}
...
For simulations at physiological temperatures and below, the LTC integrates quantum noise while maintaining computational efficiency. Optimised parameters include a Matsubara cutoff of $N_{\mathrm{Mat}} = 12$ for $T = \SI{295}{\kelvin}$, a time-step enhancement factor $\eta_{\mathrm{LTC}} = 10$, a convergence tolerance $\epsilon_{\mathrm{LTC}} = 10^{-8}$ for auxiliary state truncation, and a memory kernel truncation at \SI{20}{\pico\second} (beyond system decoherence timescales). This computational efficiency enables high-throughput screening of OPV transmission functions and disorder ensembles essential for realistic agrivoltaic design optimisation.
...
All simulations were performed on an AMD EPYC 7542 processor (32 cores at \SI{2.9}{\giga\hertz}) with \SI{256}{\giga\byte} DDR4-3200 RAM, running Ubuntu 20.04~LTS. Code was compiled with GCC~9.4.0 using \texttt{-O3} optimisation. Parallel ensemble runs used OpenMPI~4.0.5.

\subsection{Scaling analysis}

Typical production runs for the 7-site FMO system (\SI{100}{\pico\second}, $\Delta t = \SI{1.0}{\femto\second}$) utilise 12 Matsubara modes and an adaptive hierarchy (averaging \numrange{800}{1200} states), requiring approximately 3.8~hours and \SI{4.2}{\giga\byte} of peak memory on a single core. Scaling analysis demonstrates that adHOPS remains size-invariant for localised excitons [$\mathcal{O}(1)$], whereas HEOM exhibits $\mathcal{O}(N^3)$ scaling, making adHOPS the preferred method for $N > 5$.

\subsection{Parallelization efficiency}

Ensemble averaging over 100 independent disorder realizations demonstrates near-perfect parallel scalability: a total wall time of 4 hours on 100 cores, compared to 16.7 days on a single core.
